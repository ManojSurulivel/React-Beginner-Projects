...
const App = () => {
  ...
  return (
    <div class="min-h-screen bg-gray-100 flex flex-col items-center justify-start pt-10 px-4">
      <h1 class="text-3xl font-bold text-purple-700 mb-4">Todo List App</h1>

      <div class="flex flex-col sm:flex-row gap-4 w-full max-w-md items-center">
        <input
          type="text"
        ...
        <button
          onClick={handleAdd}
          class="bg-purple-600 hover:bg-purple-700 text-white px-6 py-2 rounded-md w-full sm:w-auto"
        >
          Add
        </button>
      </div>
      ...
      <ul class="mt-6 w-full max-w-md">
        {display.map((value, index) => (
          <li
            key={index}
            class="flex justify-between items-center bg-white shadow-md p-4 rounded-md mb-3"
          >
